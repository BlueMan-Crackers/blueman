...
app.put('/api/products/:Sno', (req, res) => {
  ...
  try {
    ...
    const result = update.run(price, discount, Sno);
...
app.post('/api/products', (req, res) => {
  ...
  try {
    const insert = db.prepare("INSERT INTO products (id, name, category, image, price, discount) VALUES (?, ?, ?, ?, ?, ?)");
    const result = insert.run(id || null, name, category, defaultImage, price, finalDiscount);
...
app.delete('/api/products/category/:category', (req, res) => {
  ...
  try {
    ...
    res.json({ message: "Category deleted", changes: result.changes });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});
